...
<style>
    ...
    table.data-table thead th { text-align: left; font-size: 13px; font-weight: 600; color: #6b7280; padding: 14px 24px; }
    table.data-table thead th.actions-col { text-align: right; }
...
<div class="page-header">
    <h1>Branches</h1>
    <p>Manage your store branches here.</p>
</div>

<div class="toolbar">
    <div class="toolbar-left">
        <form method="GET">
            <input type="text" name="search" value="{{ request('search') }}" class="search-input" placeholder="Search branch...">
        </form>
        <div class="record-badge">{{ $branches->count() }} records</div>
    </div>
    <a href="{{ route('branches.create') }}" class="btn-add">+ Add Branch</a>
</div>
...
<div class="table-wrap">
    <table class="data-table">
        ...
        <tbody>
            @forelse ($branches as $branch)
                <tr>
                    ...
                    <td class="actions-cell">
                        <a href="{{ route('branches.edit', $branch) }}" class="icon-btn edit" title="Edit">
                        ...
                        <form action="{{ route('branches.destroy', $branch) }}" method="POST" style="display:inline;" onsubmit="return confirm('Delete this branch?');">
                            @csrf @method('DELETE')
            ...
            @empty
                <tr><td colspan="2" class="empty-row">No branches found.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
