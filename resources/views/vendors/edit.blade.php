        ...
        <div class="modal-card-header">
            <div class="modal-header-left">
                <div class="modal-icon-badge">
                    <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a4 4 0 00-5-3.87M9 20H4v-2a4 4 0 015-3.87m6-4a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                </div>
                <div>
        ...
        <form method="POST" action="{{ route('vendors.update', $vendor) }}">
            ...
            <div class="modal-body">
                <div class="form-group" style="margin-bottom:20px;">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $vendor->name) }}" class="{{ $errors->has('name') ? 'is-invalid' : '' }}" autofocus>
                    @error('name')<div class="error-text">{{ $message }}</div>@enderror
                </div>
                <div class="form-group" style="margin-bottom:20px;">
                    <label for="phone">Vendor Phone Number</label>
                    <input type="text" id="phone" name="phone" value="{{ old('phone', $vendor->phone) }}" class="{{ $errors->has('phone') ? 'is-invalid' : '' }}">
                    @error('phone')<div class="error-text">{{ $message }}</div>@enderror
                </div>
                <div class="form-group">
                    <label for="cnic">Vendor CNIC</label>
                    <input type="text" id="cnic" name="cnic" value="{{ old('cnic', $vendor->cnic) }}" class="{{ $errors->has('cnic') ? 'is-invalid' : '' }}">
                    @error('cnic')<div class="error-text">{{ $message }}</div>@enderror
                </div>
            </div>
