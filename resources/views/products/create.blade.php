...
<style>
    ...
    .type-btn.active { border-color: #f5a524; background: #fffbf0; color: #92400e; }
    ...
    .form-group input:focus, .form-group select:focus { outline: none; border-color: #f5a524; box-shadow: 0 0 0 3px rgba(245,165,36,0.15); }
    ...
    .btn-add-item:hover { border-color: #f5a524; color: #92400e; background: #fffbf0; }
    ...
    .upload-dropzone img.upload-preview {
        width: 100%;
        height: 100%;
        object-fit: cover;
    }
...
<div class="modal-page-wrap">
    <div class="modal-card">
        ...
        <form method="POST" action="{{ route('products.store') }}" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="type" id="type-input" value="{{ old('type', 'accessory') }}">
            ...
            <div class="modal-body">
                ...
                <div>
                    ...
                    <div class="type-toggle">
                        <button type="button" class="type-btn {{ old('type', 'accessory') === 'accessory' ? 'active' : '' }}" onclick="setType('accessory')">🔌 Accessory</button>
                        <button type="button" class="type-btn {{ old('type') === 'mobile' ? 'active' : '' }}" onclick="setType('mobile')">📱 Mobile</button>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="fields-grid">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="quantity" value="{{ old('quantity') }}" min="1" class="{{ $errors->has('quantity') ? 'is-invalid' : '' }}">
                            @error('quantity')<div class="error-text">{{ $message }}</div>@enderror
                        </div>

                        {{-- Total Purchase Amount — Mobile only, shown in Product Info --}}
                        <div class="form-group" id="total-purchase-wrap" style="{{ old('type', 'accessory') !== 'mobile' ? 'display:none' : '' }}">
                            <label>Total Purchase Amount</label>
                            <input type="number" name="total_purchase_amount" value="{{ old('total_purchase_amount') }}" step="0.01" min="0" class="{{ $errors->has('total_purchase_amount') ? 'is-invalid' : '' }}">
                ...
                {{-- Accessory Fields --}}
                <div id="accessory-fields" style="{{ old('type', 'accessory') === 'mobile' ? 'display:none' : '' }}">
                    <p class="section-title">Accessory Details</p>
                    <div class="fields-grid">
                        <div class="form-group">
                            <label>Purchase Price</label>
                            <input type="number" name="purchase_price" value="{{ old('purchase_price') }}" step="0.01" min="0" class="{{ $errors->has('purchase_price') ? 'is-invalid' : '' }}">
                            @error('purchase_price')<div class="error-text">{{ $message }}</div>@enderror
                        </div>
                        <div class="form-group">
                            <label>Sell Price</label>
                            <input type="number" name="sell_price" value="{{ old('sell_price') }}" step="0.01" min="0" class="{{ $errors->has('sell_price') ? 'is-invalid' : '' }}">
                ...
                {{-- Mobile Item Cards --}}
                <div id="mobile-fields" style="{{ old('type', 'accessory') !== 'mobile' ? 'display:none' : '' }}">
                            ...
                            <div class="item-card">
                                ...
                                <div class="fields-grid">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="items[{{ $i }}][reg_status]" class="{{ $errors->has("items.{$i}.reg_status") ? 'is-invalid' : '' }}">
                                        ...
                                        @error("items.{$i}.reg_status")<div class="error-text">{{ $message }}</div>@enderror
                                    </div>
                                    <div class="form-group">
                                        <label>Branch</label>
                                        <select name="items[{{ $i }}][branch_id]" class="{{ $errors->has("items.{$i}.branch_id") ? 'is-invalid' : '' }}">
                                            <option value="">— Select Branch —</option>
                                            @foreach ($branches as $branch)
                                                <option value="{{ $branch->id }}" {{ ($item['branch_id'] ?? '') == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                                            @endforeach
                                        </select>
                                        @error("items.{$i}.branch_id")<div class="error-text">{{ $message }}</div>@enderror
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group upload-group">
                                        ...
                                        <input type="file" id="item-image-{{ $i }}" name="items[{{ $i }}][image]" accept="image/*"
                                            class="upload-input-hidden {{ $errors->has("items.{$i}.image") ? 'is-invalid' : '' }}"
                                            onchange="previewImage(this, 'item-dropzone-{{ $i }}')">
                                        @error("items.{$i}.image")<div class="error-text">{{ $message }}</div>@enderror
                                    </div>
                                </div>
                            </div>
                        @endforeach
...
<script>
    ...
    function previewImage(input, dropzoneId) {
        ...
        reader.onload = function (e) {
            dropzone.innerHTML = `<img src="${e.target.result}" class="upload-preview">`;
        };
